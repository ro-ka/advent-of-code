...
fn main() {
  let raw_entries = read_file().unwrap();
...
fn part1(entries: &Vec<Entry>) {
  let valid_passwords = entries.iter()
    .filter(|entry| {
      let letter_count = entry.password.chars().filter(|c| c == &entry.letter).count();
...
fn read_file() -> std::io::Result<Vec<String>> {
  let data = fs::read_to_string("input.txt")?.lines().map(|v| v.to_string()).collect();
  return Ok(data);
}
