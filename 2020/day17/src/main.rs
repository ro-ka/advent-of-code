...
fn part1(entries: &Vec<String>) {
    ...
    for _ in 0..6 {
        state = cycle_3_dimensions(state.clone());
    }
...
fn part2(entries: &Vec<String>) {
    let mut state: Vec<(i32, i32, i32, i32)> = vec![];

    entries.iter().enumerate().for_each(|(x, row)| {
        row.chars().enumerate().for_each(|(y, c)| {
            if c == '#' {
                state.push((x as i32, y as i32, 0i32, 0i32));
            }
        });
    });

    for _ in 0..6 {
        state = cycle_4_dimensions(state.clone());
    }

    println!("Part 2: {:?}.", state.len());
}

fn cycle_3_dimensions(state: Vec<(i32, i32, i32)>) -> Vec<(i32, i32, i32)> {
...
fn cycle_4_dimensions(state: Vec<(i32, i32, i32, i32)>) -> Vec<(i32, i32, i32, i32)> {
    let mut new_state: Vec<(i32, i32, i32, i32)> = vec![];

    let x_values = state.iter().map(|(x, _, _, _)| x);
    let y_values = state.iter().map(|(_, y, _, _)| y);
    let z_values = state.iter().map(|(_, _, z, _)| z);
    let w_values = state.iter().map(|(_, _, _, w)| w);

    for x in (x_values.clone().min().unwrap() - 1)..=(x_values.clone().max().unwrap() + 1) {
        for y in (y_values.clone().min().unwrap() - 1)..=(y_values.clone().max().unwrap() + 1) {
            for z in (z_values.clone().min().unwrap() - 1)..=(z_values.clone().max().unwrap() + 1) {
                for w in
                    (w_values.clone().min().unwrap() - 1)..=(w_values.clone().max().unwrap() + 1)
                {
                    let key = (x, y, z, w);
                    let currently_active = state.iter().find(|k| **k == key).is_some();
                    let active_neighbors = state
